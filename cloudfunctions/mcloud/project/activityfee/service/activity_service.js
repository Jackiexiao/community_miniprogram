...
class ActivityService extends BaseProjectService {
	...
	async prepay(userId, activityId, forms) {
		...
		// 检查是否可以报名
		let timestamp = this._timestamp;
		if (activity.ACTIVITY_STATUS == 0) this.AppError('该活动已停止');
		if (activity.ACTIVITY_END <= timestamp) this.AppError('该活动已结束');
		if (activity.ACTIVITY_STOP <= timestamp) this.AppError('该活动已截止报名');
		if (activity.ACTIVITY_MAX_CNT > 0 && activity.ACTIVITY_JOIN_CNT >= activity.ACTIVITY_MAX_CNT) this.AppError('该活动报名已满');
		...
		if (cnt > 0) this.AppError('您已经报名，请不要重复报名');
		...
		let data = {
			...
			ACTIVITY_JOIN_CODE: dataUtil.genRandomString(15),
			ACTIVITY_JOIN_IS_CHECKIN: 0,

			ACTIVITY_JOIN_PAY_STATUS: 99, // 99=无需支付
			ACTIVITY_JOIN_PAY_FEE: 0,  // 费用为0
		};
		...
		if (activity.ACTIVITY_METHOD == 0 || activity.ACTIVITY_FEE == 0) {
			return {
				activityJoinId,
				payRet: null, // 不返回支付信息
				check: activity.ACTIVITY_CHECK_SET // 返回是否需要审核的状态
			};
		}

		// 如果需要支付（后续扩展用）
		let payService = new PayService();
		return await payService.createPay(userId, activityJoinId, activityId, activity.ACTIVITY_FEE);
	}
